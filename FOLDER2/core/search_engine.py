...
# This is the refactored version of conduct_tmep_704_02_search() from the original
# search_authority_engine.py. The logic is identical — only the dependencies changed:

#   Original                        New
#   ─────────────────────────────   ──────────────────────────────────────────
#   _validate_application()      →  validators.validate_and_parse()
#   _build_search_queries()      →  query_builder.build_search_queries()
#   UsptoTessAdapter (stub)      →  adapters.tess_live.TessLiveAdapter (real API)
#   MockConflictAdapter (inline) →  adapters.mock.MockConflictAdapter (imported)
#   All constants                →  core.models

# Architecture:
#     § 704.02 Search Engine  →  Returns conflict set
#             ↓
#     § 1207 Similarity Engine  →  Evaluates likelihood   [NOT THIS MODULE]
#             ↓
#     Office Action Generator                             [NOT THIS MODULE]
# """
...
# from core.query_builder import build_search_queries
# from core.validators import (
#     ApplicationValidationError,   # re-exported for backward compat
#     SearchNotRequiredError,        # re-exported for backward compat
#     assert_search_required,
#     evaluate_re_search,
#     validate_and_parse,
# )
...
#     TMEP §704.02 — Examining Attorney's Search
#     ============================================
#     Mandatory procedural search engine.

#     Args:
#         application_data  : Raw trademark application dict (see schema below).
#         tess_adapter      : Optional adapter override.
#                             Defaults to TessLiveAdapter() — the real USPTO API.
#                             Pass MockConflictAdapter() in tests.
#         return_audit_log  : If True, returns (output_dict, audit_log_dict) tuple.

#     Returns:
#         dict — structured compliance JSON.
#         tuple[dict, dict] — if return_audit_log=True.

#     Raises:
#         ApplicationValidationError — required fields missing or wrong type.
#         SearchNotRequiredError     — event trigger does not mandate a §704.02 search.
...
#             "mark_text":          str,
#             "mark_type":          str,
#             "goods_services":     [{"class": str, "description": str}],
#             "event_trigger":      str,   # see EventTrigger enum
...
#     # ── 3. TIMESTAMP (UTC ISO-8601) ───────────────────────────────────────────
#     search_timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

#     # ── 4. BUILD SOLR QUERIES ─────────────────────────────────────────────────
#     queries = build_search_queries(app.mark_text, app.goods_services)

#     # ── 5. EXECUTE AGAINST USPTO TESS (live API or injected adapter) ──────────
#     # adapter   = tess_adapter or TessLiveAdapter()
#     # conflicts = adapter.search(queries)
#     # ── 5. EXECUTE AGAINST USPTO TESS (live API or injected adapter) ──────────
#     if tess_adapter is not None:
#         adapter = tess_adapter
#     else:
#         adapter = TessLiveAdapter()

#     conflicts = adapter.search(queries)

#     # ── 6. RE-SEARCH FLAG (§718.07) ───────────────────────────────────────────
#     re_search_required, re_search_basis = evaluate_re_search(app.event_trigger)

#     # ── 7. COMPLIANCE STATUS STRING ───────────────────────────────────────────
#     compliance_status = _build_compliance_status(
#         event_trigger      = app.event_trigger,
#         search_conducted   = True,
#         conflicts_found    = len(conflicts),
...
#         "results_summary": {
#             "total_conflicts_found":           len(conflicts),
#             "conflicting_application_numbers": [
#                 c.application_number for c in conflicts
#             ],
#         },
#         "re_search_required": re_search_required,
#         "compliance_status":  compliance_status,
...
# # ──────────────────────────────────────────────────────────────────────────────
# # INTERNAL HELPERS  (unchanged logic from search_authority_engine.py)
# # ──────────────────────────────────────────────────────────────────────────────
...
#         f"Procedural search completed under {AUTHORITY_REFERENCE}",
#         f"triggered by {trigger_label}.",
#         f"Search conducted: {'YES' if search_conducted else 'NO'}.",
#         f"Total conflicts identified: {conflicts_found}.",
#     ]
