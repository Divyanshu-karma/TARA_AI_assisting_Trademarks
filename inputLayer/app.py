    ...
    if result["status"] == "ok":
        ...
        if st.button("⚖️ Run Full Examination (Pillar 1–3)", type="primary"):
            try:
                ...
                from adapters.mock import MockConflictAdapter  # or TessLiveAdapter

        # run_full_pipeline returns a dict, not a state object
                pipeline_result = run_full_pipeline(
                result["data"],
                tess_adapter = MockConflictAdapter(),
                save_result  = True,
                )

                st.subheader("Pipeline Structural Summary")
                st.write("Status:",            pipeline_result["pipeline_status"])
                st.write("Structurally Clean:", pipeline_result["structurally_clean"])
                st.write("Engines Run:",        pipeline_result["run_engines"])
                st.write("Engines Blocked:",    pipeline_result["blocked_engines"])
                st.write("Partial Refusals:",   pipeline_result["pillar3_summary"]["partial_refusal_classes"])
                st.write("Division Candidates:", pipeline_result["pillar3_summary"]["division_candidates"])

                st.subheader("Pillar 1 Summary")
                st.json(pipeline_result["pillar1_summary"])

                st.subheader("Gate Flags")
                st.json(pipeline_result["gate_flags"])

                if pipeline_result.get("aggregated_refusals"):
                    st.subheader("Aggregated Refusals")
                    st.json(pipeline_result["aggregated_refusals"])
