...
#     return None
def parse_date_flexible(date_str: str) -> Optional[date]:
    """Try all known USPTO date formats. Strips time/timezone if present."""
    if not date_str or not str(date_str).strip():
        return None
    cleaned = str(date_str).strip()
    
    # Strip common time/timezone suffixes: " at 12:13:12 PM ET"
    cleaned = re.sub(r'\s+at\s+\d{1,2}:\d{2}:\d{2}\s+(AM|PM)?\s*[A-Z]*', '', cleaned, flags=re.IGNORECASE)
...
def rule_fee_amount_reasonable(
    ...
    if class_count == 0:
        return ValidationResult(
            field="total_fee_paid", rule="fee_amount_reasonable", passed=False,
            severity="ERROR", message="Class count is 0 — cannot validate fee.",
            extracted_value=total_fee, confidence_penalty=0.5,
        )

    expected_plus = TEAS_PLUS_FEE_PER_CLASS * class_count
    expected_std = TEAS_STANDARD_FEE_PER_CLASS * class_count

    # Smart Override: If fee perfectly matches Standard, but filing_type says Plus
    if float(total_fee) == expected_std and filing_type == "TEAS_PLUS":
        return ValidationResult(
            field="total_fee_paid", rule="fee_amount_reasonable", passed=True,
            severity="WARNING", 
            message=f"Fee ${total_fee:.2f} matches TEAS_STANDARD but filing type was extracted as {filing_type}. Assuming TEAS_STANDARD.",
            extracted_value=total_fee, confidence_penalty=0.1,
        )
    ...
    if expected is None:
        return ValidationResult(
            field="total_fee_paid", rule="fee_amount_reasonable", passed=True,
            severity="INFO", message=f"Unknown filing type '{filing_type}' — fee check skipped.",
            extracted_value=total_fee,
        )
    ...
    return ValidationResult(
        field="total_fee_paid", rule="fee_amount_reasonable", passed=passed,
        severity="WARNING",
        message=f"Fee ${total_fee:.2f} does not match expected ${expected:.2f} for {class_count} class(es) at {filing_type}." if not passed else f"Fee ${total_fee:.2f} matches expected.",
        extracted_value=total_fee, confidence_penalty=0.5 if not passed else 0.0,
    )
