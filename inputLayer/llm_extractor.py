...
10.For filing_type: extract the exact application type, e.g., "TEAS_PLUS" or "TEAS_STANDARD".
...
def coerce_types(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_filing_type = to_str(data.get("filing_type")).upper().replace(" ", "_")
    
    # Infer filing type from fee if missing or invalid
    if raw_filing_type not in ["TEAS_PLUS", "TEAS_STANDARD", "TEAS_RF"]:
        total_fee = to_float(data.get("total_fee_paid"))
        count = to_int(data.get("fees_paid_count"), default=1)
        if total_fee > 0 and count > 0:
            fee_per_class = total_fee / count
            if fee_per_class == 350.0:
                raw_filing_type = "TEAS_STANDARD"
            elif fee_per_class == 250.0:
                raw_filing_type = "TEAS_PLUS"
            else:
                raw_filing_type = "UNKNOWN"
        else:
            raw_filing_type = "UNKNOWN"
    data["filing_type"]        = raw_filing_type
